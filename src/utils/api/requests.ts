...
const putRequest = (url: string) => (token?: string) => (body: object) =>
  fetcher(url, token, 'PUT', body);
const postRequest = (url: string) => (token?: string) => (body: object) =>
...
const putDataSurveyUnitById =
  (apiUrl: string) => (id: string) => (token?: string) => (body: object) =>
