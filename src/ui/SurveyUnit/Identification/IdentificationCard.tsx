    ...
    <Card elevation={0}>
      <CardContent>
        <Stack gap={3}>
          ...
          <Stack gap={1}>
            {(Object.keys(questions.map) as IdentificationQuestionsId[]).map(questionId => (
              <ButtonLine
                key={questionId}
                onClick={() => setSelectedDialogId(questionId)}
                label={responses[questionId]?.label ?? questions?.map[questionId]?.text}
      ...
      {selectedDialogId && (
        <IdentificationDialog
          ...
          question={questions.map[selectedDialogId]}
