...
export const synchronizePearl = async (transactionMonitor: Transaction | undefined) => {
  ...
  try {
    const uploadTransaction = transactionMonitor?.startSpan(
      '[pilotage] synchronisation - upload local data'
    );
    await getUserData();
    surveyUnitsInTempZone = await sendData();
    transmittedSurveyUnits = await getWFSSurveyUnitsSortByCampaign();
    uploadTransaction?.end();
    const cleanTransaction = transactionMonitor?.startSpan(
      '[pilotage] synchronisation - clean local data'
    );
    await clean();
    cleanTransaction?.end();

    const downloadTransaction = transactionMonitor?.startSpan(
      '[pilotage] synchronisation - download server data'
    );
