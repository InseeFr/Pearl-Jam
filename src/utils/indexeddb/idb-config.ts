...
import { contactOutcomes, specificContactOutcomes } from 'utils/functions/contacts/ContactOutcome';
...
const convertDeprecatedContactOutcomeType = (contactOutcomeType: string) => {
  ...
  if (contactOutcomeType === specificContactOutcomes.DECEASED.value)
    newContactOutcomeType = contactOutcomes.NOT_APPLICABLE.value;
  else if (
    contactOutcomeType === specificContactOutcomes.DEFINITLY_UNAVAILABLE_FOR_UNKNOWN_REASON.value
  ) {
