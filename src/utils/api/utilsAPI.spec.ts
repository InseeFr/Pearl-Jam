...
describe('utilsAPI', () => {
  ...
  it('should perform health check successfully', async () => {
    ...
    await expect(healthCheck(urlPearApi, authenticationMode)).resolves.toBe('Health check passed');
    expect(authentication).toHaveBeenCalledWith(authenticationMode);
    expect(getToken).toHaveBeenCalled();
  ...
  it('should throw an error when health check fails', async () => {
    ...
    await expect(healthCheck(urlPearApi, authenticationMode)).rejects.toThrowError();
  });
